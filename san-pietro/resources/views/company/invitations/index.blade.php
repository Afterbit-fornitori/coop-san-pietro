                    ...
                    <tbody class="bg-white dark:bg-gray-700 divide-y divide-gray-200 dark:divide-gray-600">
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-sm text-gray-500">{{ $invitation->business_type }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $invitation->email }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $invitation->sector }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <a href="{{ route('invitations.show', $invitation) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                    Dettagli
                                </a>
                                @if($invitation->isPending() && !$invitation->isExpired())
                                <form action="{{ route('invitations.resend', $invitation) }}" method="POST" class="inline mr-3">
                                    @csrf
