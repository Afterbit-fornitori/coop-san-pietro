...
<div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6 text-gray-900 dark:text-gray-100">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold">Modifica Registrazione Carico/Scarico</h2>
        ...
        <form action="{{ route('loading-unloading.update', $loadingUnloadingRegister) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="data_operazione" id="data_operazione"
                        value="{{ old('data_operazione', \Carbon\Carbon::parse($loadingUnloadingRegister->data_operazione)->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <select name="tipo_operazione" id="tipo_operazione" required
                        ...
                        <option value="CARICO" {{ old('tipo_operazione', $loadingUnloadingRegister->tipo_operazione) === 'CARICO' ? 'selected' : '' }}>CARICO</option>
                        <option value="SCARICO" {{ old('tipo_operazione', $loadingUnloadingRegister->tipo_operazione) === 'SCARICO' ? 'selected' : '' }}>SCARICO</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="product_id" id="product_id" required
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" {{ old('product_id', $loadingUnloadingRegister->product_id) == $product->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="lotto" id="lotto" value="{{ old('lotto', $loadingUnloadingRegister->lotto) }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500">
                </div>
            </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="kg_reimmersione" id="kg_reimmersione"
                            value="{{ old('kg_reimmersione', $loadingUnloadingRegister->kg_reimmersione) }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500">
                    </div>

                    <div>
                        <label for="kg_piccola" class="block text-sm font-medium mb-2">Piccola (kg)</label>
                        <input type="number" step="0.01" name="kg_piccola" id="kg_piccola"
                            value="{{ old('kg_piccola', $loadingUnloadingRegister->kg_piccola) }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500">
                    </div>

                    <div>
                        <label for="kg_media" class="block text-sm font-medium mb-2">Media (kg)</label>
                        <input type="number" step="0.01" name="kg_media" id="kg_media"
                            value="{{ old('kg_media', $loadingUnloadingRegister->kg_media) }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500">
                    </div>

                    <div>
                        <label for="kg_grossa" class="block text-sm font-medium mb-2">Grossa (kg)</label>
                        <input type="number" step="0.01" name="kg_grossa" id="kg_grossa"
                            value="{{ old('kg_grossa', $loadingUnloadingRegister->kg_grossa) }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500">
                    </div>

                    <div>
                        <label for="kg_granchio" class="block text-sm font-medium mb-2">Granchio (kg)</label>
                        <input type="number" step="0.01" name="kg_granchio" id="kg_granchio"
                            value="{{ old('kg_granchio', $loadingUnloadingRegister->kg_granchio) }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="provenienza_destinazione" id="provenienza_destinazione"
                    value="{{ old('provenienza_destinazione', $loadingUnloadingRegister->provenienza_destinazione) }}"
            ...
            <div>
                ...
                <select name="transport_document_id" id="transport_document_id"
                    ...
                    @foreach($transportDocuments as $doc)
                        <option value="{{ $doc->id }}" {{ old('transport_document_id', $loadingUnloadingRegister->transport_document_id) == $doc->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea name="note" id="note" rows="3"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500">{{ old('note', $loadingUnloadingRegister->note) }}</textarea>
            </div>
